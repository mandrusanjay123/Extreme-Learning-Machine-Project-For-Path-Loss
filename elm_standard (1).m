...
% C is for regularisation factor Ridge regression
...
bhalue = ceil(Opts.Tr_ratio*size(Xdata,1));
if Opts.fixed==1
    
    X=Xdata(1:bhalue,:);        % X  :    training inputs 
    Xts=Xdata(bhalue+1:end,:);    %Y  :    training targets
    Y=Ydata(1:bhalue,:);         %Xts:    testing inputs
    Yts=Ydata(bhalue+1:end,:);     %Yts:    testing targets
else
...
%B=(eye(size(H,1))*C+H * H')\H*Y; 
B = pinv(H*H'+C*eye(size(H,1)))*H*Y;
%      
    else
   %B=(((eye(size(H,2))*C+H' * H) \H')'*Y);  
   B = (pinv(H'*H+C*eye(size(H,2)))*H')'*Y;
...
p=randperm(size(x,1));
h=round(trainRatio*size(x,1));
Inputs=x(p(1:h),:);
TsInputs=x(p(h+1:end),:);
Targets=y(p(1:h),:);
TsTargets=y(p(h+1:end),:);
% bvalue=ceil(trainRatio*size(x,1));
% Inputs=x(1:bvalue,:);
% Targets=y(1:bvalue,:);
% TsInputs=x(bvalue+1:end,:);
% TsTargets=y(bvalue+1:end,:);
